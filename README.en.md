...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/Nymph/wiki/Home) | [English Home](https://github.com/alkaid616/Nymph/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `Nymph`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `Nymph.json` and update `id`, `name`, `author`, and `description`.
3. Edit `NymphCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `Nymph/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `Nymph.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `Nymph.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\Nymph.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/Nymph` directory.
...
Nymph/
├── NymphCode/   # C# source
├── Nymph/       # Godot resources, localization, and placeholder scenes
├── Nymph.csproj
...
`res://Nymph/...` is the Godot/PCK resource path, mapping to the repository resource directory `Nymph/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
|---|---|
| Type | `NymphCharacter` |
| Expected ID | `NYMPH_CHARACTER_NYMPH_CHARACTER` |
| Starter deck | 4 × `NymphStrike`, 4 × `NymphDefend`, 1 × `NymphRelic` |
...
|---|---|---|
| `NymphStrike` (attack) | character card pool | `NYMPH_CARD_NYMPH_STRIKE` |
| `NymphDefend` (skill) | character card pool | `NYMPH_CARD_NYMPH_DEFEND` |
| `NymphRelic` | `NymphRelicPool` | `NYMPH_RELIC_NYMPH_RELIC` |

### Static placeholder assets

**Images** (`res://Nymph/images/...`):

- `cards/NymphStrike.png`, `cards/NymphDefend.png`: sample card art.
- `relics/NymphRelic.png`: sample relic icon.
- `characters/Nymph_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://Nymph/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `Nymph_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `Nymph_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `Nymph_merchant.tscn` | Merchant | — |
| `Nymph_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `Nymph_character_select_bg.tscn` | Character select background | — |
...
`Nymph.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
